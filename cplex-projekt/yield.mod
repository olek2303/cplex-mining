...
int ForecastDemand[1..3][1..3][Klasy];  // Prognozowany popyt
int ActualDemand[1..3][Klasy];  // Rzeczywisty popyt
...
dvar float revenue;  // Ca³kowity przychód

// Maksymalizacja przychodu
maximize sum(p in 1..3, o in 1..3, i in Klasy) 
...
subject to {
    ...
    forall(p in 1..3, i in Klasy, j in Klasy: abs(ord(i) - ord(j)) == 1)
        x[p][i] <= 1.1 * x[p][j];
}
